...
function sortHotelsPricingLowToHigh(hotel1, hotel2) {
  return hotel1.price - hotel2.price;
}
function sortHotelsPricingHighToLow(hotel1, hotel2) {
  return hotel2.price - hotel1.price;
}
app.get('/hotels/sort/pricing', (req, res) => {
  let hotelsCopy = hotels.slice();
  let sortOrder = req.query.pricing;
  if (sortOrder === 'low-to-high') {
    hotelsCopy.sort(sortHotelsPricingLowToHigh);
  } else if (sortOrder === 'high-to-low') {
    hotelsCopy.sort(sortHotelsPricingHighToLow);
  }
  res.json({ hotels: hotelsCopy });
});

function sortHotelsRatinggLowToHigh(hotel1, hotel2) {
  return hotel1.rating - hotel2.rating;
}
function sortHotelsRatingHighToLow(hotel1, hotel2) {
  return hotel2.rating - hotel1.rating;
}
app.get('/hotels/sort/rating', (req, res) => {
  let hotelsCopy = hotels.slice();
  let sortOrder = req.query.rating;
  if (sortOrder === 'low-to-high') {
    hotelsCopy.sort(sortHotelsRatinggLowToHigh);
  } else if (sortOrder === 'high-to-low') {
    hotelsCopy.sort(sortHotelsRatingHighToLow);
  }
  res.json({ hotels: hotelsCopy });
});

function sortHotelsReviewsLeastToMost(hotel1, hotel2) {
  return hotel1.reviews - hotel2.reviews;
}
function sortHotelsReviewsMostToLeast(hotel1, hotel2) {
  return hotel2.reviews - hotel1.reviews;
}
app.get('/hotels/sort/reviews', (req, res) => {
  let hotelsCopy = hotels.slice();
  let sortOrder = req.query.reviews;
  if (sortOrder === 'least-to-most') {
    hotelsCopy.sort(sortHotelsReviewsLeastToMost);
  } else if (sortOrder === 'most-to-least') {
    hotelsCopy.sort(sortHotelsReviewsMostToLeast);
  }
  res.json({ hotels: hotelsCopy });
});
